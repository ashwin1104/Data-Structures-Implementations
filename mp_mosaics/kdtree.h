...
private:
  ...
  Point<Dim> getPotentialNearest(bool go_left, const Point<Dim> &query, typename KDTree<Dim>::KDTreeNode *subRoot, Point<Dim> potential_nearest, int d) const;

  int sortByMedian(int start_index, int end_index, int d, int pivot_index);
